    ...
    if (page) {
        ...
        if ([photo underlyingImage]) {
            ...
            if (photo.imageSequenceURLs && !photo.imageSequenceView) {
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:kAlertWarningTitle message:kAlertNoInternetBody preferredStyle:UIAlertControllerStyleAlert];
...
- (void)configurePage:(IDMZoomingScrollView *)page forIndex:(NSUInteger)index {
    ...
    [page.progressView setFrame:CGRectMake((kScreenWidth-35.)/2., (kScreenHeight-35.)/2, 35.0f, 35.0f)];
}
